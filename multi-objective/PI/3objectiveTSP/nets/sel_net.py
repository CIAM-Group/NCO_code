...
class Sel_net(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, n_heads, n_layers, num_obj, normalization, device):
        ...
        self.project_x = nn.Sequential(
            nn.Linear(embedding_dim, self.embedding_dim),
            nn.ReLU(),
            nn.Linear(self.hidden_dim, 1)
        )
        self.decoder = MultiHeadSelDecoder(input_dim=self.embedding_dim, embed_dim=self.embedding_dim)

    def forward(self, batch, f, vec, his, do_sample=False):
        bs, num, os = f.size()
        outdim = vec.size(-1)
        ...
        x = self.project_x(fusion).squeeze(-1)
        if his is not None:
            x[torch.arange(bs), his[:]] = -np.inf
        logval = F.log_softmax(x, dim=-1)
        val = F.softmax(x, dim=-1)
        _ = val.multinomial(1) if do_sample else val.max(-1)[1].view(-1, 1)
        ans = torch.gather(logval, dim=1, index=_)
        return _.squeeze(-1), ans
