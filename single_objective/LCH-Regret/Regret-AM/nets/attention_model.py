...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        _ = self.regret_embedding[None, None, :].expand(embeddings.size(0), 1, embeddings.size(-1))
        embeddings = torch.cat((embeddings, _), dim=1)
        _log_p, pi = self._inner(input, embeddings)

        cost, mask = self.problem._get_travel_distance(input, pi)
    ...
    def _calc_log_likelihood(self, _log_p, a, mask):
        ...
        m1 = (a == _log_p.size(-1) - 1)  # regret的那一步
        m2 = (m1.roll(dims=-1, shifts=-1) | m1)  # regret的那一步和后悔掉的那一步 路径长度都不加
        regret_mask_matrix = m1
        add_mask_matrix = (~m2)
        regret_weight = 1
        log_prob = torch.zeros((_log_p.size(0)), device=_log_p.device)
        log_p = _log_p.gather(2, a.unsqueeze(-1)).squeeze(-1)
        for t in range(log_p.shape[1]):
            # self.env.regret_mask_matrix regret那一步的mask矩阵
            # self.env.add_mask_matrix 正常加概率的mask矩阵
            regret_index = torch.nonzero(regret_mask_matrix[:, t].unsqueeze(-1))
            add_index = torch.nonzero(add_mask_matrix[:, t].unsqueeze(-1))

            log_prob[add_index[:, 0]] = log_prob[add_index[:, 0]].clone() + log_p[add_index[:, 0], t]
            log_prob[regret_index[:, 0]] = log_prob[regret_index[:, 0]].clone() + regret_weight * (
                log_p[regret_index[:, 0], t])

        # Optional: mask out actions irrelevant to objective so they do not get reinforced
        if mask is not None:
            log_prob[mask] = 0

        assert (log_prob > -1000).data.all(), "Logprobs should not be -inf, check sampling procedure!"
    ...
    def _inner(self, input, embeddings):
        ...
        outputs = []
        sequences = []
        # selected_count = torch.zeros((self.batch_size, self.pomo_size), dtype=torch.long)
    ...
    def _precompute(self, embeddings, num_steps=1):
        ...
        graph_embed = embeddings[:, :-1, :].mean(1)
        # fixed context = (batch_size, 1, embed_dim) to make broadcastable with parallel timesteps
        fixed_context = self.project_fixed_context(graph_embed)[:, None, :]

        # The projection of the node embeddings for the attention is calculated once up front0
