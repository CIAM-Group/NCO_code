...
def solve_lkh_log(executable, directory, name, loc, agents=10, runs=1, disable_cache=False):
    ...
    try:
        ...
        if os.path.isfile(output_filename) and not disable_cache:
            ...
            write_tsplib(problem_filename, agents, loc, name=name)
            ...
            with open(log_filename, 'w') as f:
                ...
                duration = time.time() - start
            tour, result = read_tsplib(tour_filename, len(loc))
            save_dataset((tour, duration), output_filename)

        return result, tour, duration
...
def write_tsplib(filename, agents, loc, name="problem"):
    with open(filename, 'w') as f:
        f.write("\n".join([
            "{} : {}".format(k, v)
            for k, v in (
...
def read_tsplib(filename, n):
    with open(filename, 'r') as f:
        result = f.read()
    result = result.split('\n')
    obj_val = float(result[1].replace('_', '.').split(' ')[-1]) / 10000000
...
if __name__ == "__main__":
    ...
    parser.add_argument("agents", type=int, help="Filename of the dataset(s) to evaluate")
    ...
    for dataset_path in opts.datasets:
        ...
        if method == "nn":
            ...
            if method == "concorde":
                ...
                def run_func(args):
                    return solve_lkh_log(executable, *args, agents=opts.agents, runs=runs, disable_cache=opts.disable_cache)
