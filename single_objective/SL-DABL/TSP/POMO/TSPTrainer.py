...
class TSPTrainer:
    ...
    def _train_one_batch_supervised(self, batch_size):
        ...
        while not done:
            ...
            # shape: (batch, pomo)
            state, reward, done = self.env.step(self.env.solutions[:, :, t])
            t += 1
            prob_list.append(prob)

        prob_list = torch.stack(prob_list, dim=-1)
        reshape_prob_list = prob_list.permute(0, 2, 1, 3).contiguous() + 1e-15
        loss1 = self.nll_loss(reshape_prob_list.log(), self.env.solutions, reduce=False)
        ...
        while not done:
            selected, prob = self.model(state)
            # shape: (batch, pomo)
            state, reward, done = self.env.step(self.env.solutions_flipped[:, :, t])
            t += 1
            prob_list.append(prob)

        prob_list = torch.stack(prob_list, dim=-1)
        reshape_prob_list = prob_list.permute(0, 2, 1, 3).contiguous() + 1e-15
        loss2 = self.nll_loss(reshape_prob_list.log(), self.env.solutions_flipped, reduce=False)

        loss = torch.stack([loss1.sum(-1), loss2.sum(-1)], dim=-1).min(-1)[0].mean()
