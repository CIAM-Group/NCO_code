...
class CVRPModel(nn.Module):
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = torch.zeros(size=(batch_size, pomo_size, pomo_size + 1))
            probs[:, :, 0] = 1
            ...
            temp = torch.cat([torch.zeros((pomo_size, 1)), torch.eye(pomo_size)], dim=1)
            probs = temp.unsqueeze(0).expand(batch_size, -1, -1)
