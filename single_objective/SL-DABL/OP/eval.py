...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if opts.multiprocessing:
        ...
        dataset = model.problem.make_dataset(filename=dataset_path, size=opts.graph_size, num_samples=opts.val_size, offset=opts.offset)
        # dataset = model.problem.make_sl_dataset(size=opts.graph_size, num_samples=opts.val_size, offset=opts.offset)
    ...
    print("Calculated total duration: {}".format(np.sum(durations) / parallelism))
...
if __name__ == "__main__":
    ...
    model_size = 100
    opts.graph_size = 100
    if opts.graph_size > 300:
        opts.val_size = 200

    dataset_path = './op_dist_test_data/op{}_{}.pt'.format(opts.graph_size, opts.val_size)
    opts.model = './models/op{}/epoch-99.pt'.format(model_size)
    print(opts.model)
    # opts.decode_strategy = 'greedy'  # 'greedy'  # 'sample'
    # widths = opts.width if opts.width is not None else [0]

    opts.decode_strategy = 'sample'
    widths = opts.width if opts.width is not None else [1280]
