...
class OP(object):
    ...
    def get_costs(dataset, pi):
        ...
        # 1e-5 for rl, 1e-4 for sl
        assert (length <= dataset['max_length'] + 1e-4).all(), \
    ...
    @staticmethod
    def make_sl_dataset(*args, **kwargs):
        return OPDataset_SL(*args, **kwargs)
...
class OPDataset_SL(Dataset):

    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0, distribution='const'):
        super(OPDataset_SL, self).__init__()
        assert distribution is not None, "Data distribution must be specified for OP"
        # Currently the distribution can only vary in the type of the prize
        prize_type = distribution
        if num_samples > 15000:
            self.data = torch.load('./training_dataset/op{}_dist_training_5w_r.pt'.format(size))
            while len(self.data) < num_samples:
                self.data += self.data
            self.data = self.data[:num_samples]
        else:
            self.data = torch.load('./training_dataset/op{}_dist_test_1w.pt'.format(size))
            # optimal = []
            # for i in range(len(self.data)):
            #     optimal.append(self.data[i][1]['max_prize'])
            # optimal = torch.tensor(optimal)
            # std, avg = torch.std_mean(optimal)
            # print('{}+-{}'.format(avg.item(), std.item()))
        self.size = len(self.data)
