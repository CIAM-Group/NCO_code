...
class CVRPPartitionTrainer:
    ...
    def run(self):
        ...
        # self.node_coords_2000, self.node_demands_2000, _, _ = self.env.make_dataset_pickle(self.trainer_params['data_load_2000'], self.trainer_params['validation_test_episodes'])
    ...
    def _test_one_batch(self, solution_gnn, solution_gnn_flag, data, demand, batch_size, episode, k):
        ...
        capacity_end2 = 1 - before.roll(dims=-1, shifts=-1).reshape(solution.size(0) * solution.size(1), -1, self.env.problem_size)[:, :, -1]
        capacity_now2 = 1 - end.reshape(solution.size(0) * solution.size(1), -1, self.env.problem_size)[:, :, 0]
