...
class CVRPPartitionTrainer:
    ...
    def _train_one_batch(self, batch_size):
        ...
        solution_flag[:, -1] = 1
        # following: DCR enabled conquering stage
        for i in range(2):
        ...
        # self.env.cal_leagal(self.env.raw_problems[:, :, -1], solution, solution_flag) #use to check legality
        # Step & Return
        ###############################################
    ...
    def gen_pyg_data(self, coors, demand, k_sparse=100, cvrplib=False):
        ...
        x = torch.stack((norm_demand, r, theta)).permute(1, 2, 0) # need to change to cpu on 10000-node problems.
