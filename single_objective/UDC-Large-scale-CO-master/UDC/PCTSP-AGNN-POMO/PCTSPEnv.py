...
import tqdm
import numpy as np
from PCTSProblemDef import get_random_problems, augment_xy_data_by_8_fold
import math
...
class PCTSPEnv:
    ...
    def load_problems(self, batch_size, subp, first, last, collected, depot_flag, aug_factor=1):
        ...
        self.last_collected = self.problems[:, :, 2][:, None, :].expand(-1, self.pomo_size, -1).gather(-1, self.pomo_last[:, :, None]).squeeze()
    ...
    def step(self, selected):
        ...
        # shape: (batch, pomo, node)
        allow = (self.load + self.last_collected + 1e-5 > 1.) & (self.depot_flag.sum(-1) == 0)
    ...
    def make_dataset_pickle(self, filename, episode):
        ...
        # shape (B,V,2)
        raw_data = torch.cat((raw_data_nodes, raw_data_prize.unsqueeze(-1), raw_data_penalty.unsqueeze(-1)), dim=-1)
        return raw_data
    ...
    def _get_travel_distance2(self, problems, solution):
        ...
        visited = visited.scatter(-1, solution, 0)
        penalty = (visited * problems[:, :, -1][:, None, :].expand(-1, solution.size(1), -1)).sum(-1)
        assert (((1 - visited) * problems[:, :, -2][:, None, :].expand(-1, solution.size(1), -1)).sum(-1) >= 1).all()
        batch_size = solution.size(0)
        pomo_size = solution.size(1)
        gathering_index = solution.unsqueeze(3).expand(batch_size, pomo_size, -1, 2)
        # shape: (batch, pomo, problem, 2)
