...
class ATSPTrainerPartition:
    ...
    def run(self):
        ...
        data_250 = torch.load('ATSP_data250_n128.pt')[:self.trainer_params['validation_test_episodes']]
    ...
    def _train_one_batch(self, batch_size):
        ...
        for i in range(2):
            ...
            tsp_insts = self.env.raw_problems[:, None, :].repeat(n_tsps_per_route.size(0), n_tsps_per_route.size(1), 1, 1).gather(-2, n_tsps_per_route.unsqueeze(-1).expand(
                -1, -1, -1, self.env.raw_problem_size)).gather(-1, n_tsps_per_route.unsqueeze(-2).expand(-1, -1, self.env.problem_size, -1))
