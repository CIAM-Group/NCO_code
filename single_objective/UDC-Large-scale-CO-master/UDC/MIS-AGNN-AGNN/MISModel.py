...
class EmbNet(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        for i in range(self.depth):
            ...
            w2 = torch.sigmoid(w0)
            now = self.agg_fn(w2 * x2[edge_index[1]], edge_index[0])
            now_reshape = torch.cat((now, torch.zeros_like(now[0:1]).repeat(x.size(0) - now.size(0), 1)), dim=0)
            x = x0 + self.act_fn(self.v_bns[i](x1 + now_reshape))
